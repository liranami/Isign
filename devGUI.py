...
def changeToDevWin(oldWin):
    oldWin.destroy()
    devGui = Tk()
    devGui.geometry("1024x600")
    devGui.title("Isign - developer")
    devGui.configure(bg="#FFFFFF")
    canvas = Canvas(devGui, bg="#FFFFFF", height=600, width=1024, bd=0, highlightthickness=0, relief="ridge")
    ...
    textarea = Text(devGui, height=18, width=40)
    ...
    chooseCamera = ttk.OptionMenu(devGui, clicked, options[0], *options)
    ...
    startBtn = Button(image=startBtn_image, borderwidth=0, highlightthickness=0,
                      command=lambda: showVideoOnGui.showVideo(clicked.get(), devGui), relief="flat")
    stopBtn = Button(image=stopBtn_image, borderwidth=0, highlightthickness=0, command=showVideoOnGui.stopShowVideo(True),
                     relief="flat")
    goBackBtn = Button(image=goBack_image, borderwidth=0, highlightthickness=0, command=lambda: main.main(devGui),
                       relief="flat")
